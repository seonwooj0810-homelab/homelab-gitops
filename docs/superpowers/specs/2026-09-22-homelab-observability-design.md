...
### R9. `tail_from_end`의 대가 — 새 Pod의 앞부분 몇 줄은 유실된다 (2026-09-23 11:20 KST)

**사실**: R8의 최종 형태(`tail_from_end = true` + positions 영속화)로 검증 5는 통과했지만,
15줄을 쓴 Pod에서 Loki에 남은 것은 **12줄**이었다. Alloy가 파일을 발견하기까지의
`local.file_match` 주기(기본 10초) 동안 쓰인 앞부분이 빠진다.

**조치와 그 한계**: `sync_period = "3s"`로 줄였다. **효과는 없었다** — 10초일 때 12/15,
3초일 때 7/10으로 양쪽 다 앞 3줄(약 6초)이 빠졌다. 지연의 지배 요인은 파일 매칭 주기가 아니라
...
오래 돌다 죽는 Pod는 Alloy가 내내 tail하고 있으므로 이 한계와 무관하다.
